                       ...
                       const u_char *data, struct dispkt *dpkt)
{
	...
	switch (dpkt->ip_proto) {
		case IPPROTO_UDP:
			dpkt->sp = ntohs(((struct udphdr *)pkt)->uh_sport);
			dpkt->dp = ntohs(((struct udphdr *)pkt)->uh_dport);
			dpkt->udp_payload_len = ntohs(((struct udphdr *)pkt)->uh_ulen) - sizeof(struct udphdr);
			pkt += 8;
		break;
		case IPPROTO_TCP:
			dpkt->sp = ntohs(((struct tcphdr *)pkt)->th_sport);
			dpkt->dp = ntohs(((struct tcphdr *)pkt)->th_dport);
			dpkt->udp_payload_len = 0;
